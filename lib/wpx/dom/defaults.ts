...
export function createDefaultDomTree(): WPXDomNode {
  return createDomNode({ id: 'wpx-root', type: 'document', name: 'WPX DOM Kernel', role: 'root', className: 'min-h-screen bg-[var(--background)] text-[var(--foreground)]', children: [createDomNode({ type: 'section', name: 'JSON-first Hero', role: 'hero', className: 'grid gap-6 rounded-3xl border border-white/10 bg-black/40 p-10', content: { eyebrow: 'WPX OS', headline: 'JSON is the DOM Kernel', body: 'Every builder action mutates the project DOM before preview or export.' }, tokens: { accent: 'color-lime' }, motion: { preset: 'reveal', duration: motionTokens.smooth }, metadata: { category: 'Hero', tags: ['json-dom', 'kernel'], status: 'stable', accessibilityStatus: 'pass', performanceStatus: 'pass' }, children: [createDomNode({ type: 'button', name: 'Primary CTA', role: 'cta', className: 'rounded-md bg-[var(--lime)] px-4 py-2 font-black text-black', content: 'Add Section' })] })] });
}
