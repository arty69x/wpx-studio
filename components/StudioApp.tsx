...
export function StudioApp() {
  ...
  return <main className="min-h-screen bg-[#0D1117] text-[#C9D1D9]">
    ...
    <div className="mx-auto grid max-w-[1800px] gap-4 p-4 lg:grid-cols-[360px_minmax(0,1fr)_390px]">
      ...
      <section className="space-y-4"><div className="rounded-2xl border border-white/10 bg-[#21262D] p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-bold text-white">Sandbox Preview Canvas</h2><p className="text-sm text-[#8B949E]">{status}</p></div><div className="flex flex-wrap gap-2"><button className="btn-ghost" onClick={undo}>Undo</button><button className="btn-ghost" onClick={redo}>Redo</button>{(["desktop", "tablet", "mobile"] as DeviceMode[]).map((item) => <button key={item} onClick={() => setDevice(item)} className={device === item ? "btn-primary" : "btn-ghost"}>{item}</button>)}<button className="btn-secondary" onClick={persist}>Save</button><button className="btn-primary" onClick={exportZip}>Export ZIP</button></div></div></div>{activeTab === "dashboard" && <Dashboard project={project} projects={projects} bytes={bytes} />}{activeTab === "assets" && <Assets project={project} uploadAsset={uploadAsset} updateProject={updateProject} />}{activeTab === "diff" && <Diff diff={diff} />}{activeTab === "graph" && <Graph project={project} />}{activeTab === "plugins" && <Plugins project={project} />}{activeTab === "dashboard" && <div className="rounded-2xl border border-white/10 bg-[#21262D] p-4"><div className="mx-auto transition-all" style={{ width: device === "desktop" ? "100%" : device === "tablet" ? 768 : 390 }}><iframe title="WPX sandbox preview" sandbox={selected.some((component) => component.scriptsEnabled) ? "allow-scripts" : ""} className="h-[620px] w-full rounded-xl border border-[#8B5CF6]/40 bg-white" srcDoc={`<!doctype html><html><head><style>${scopedCss}</style></head><body>${previewHtml || "<main style='font-family:sans-serif;padding:32px'>Import HTML to preview components.</main>"}</body></html>`} /></div></div>}</section>
...
function Assets({ project, uploadAsset, updateProject }: { project: WPXProject; uploadAsset: (event: ChangeEvent<HTMLInputElement>) => void; updateProject: (patch: Partial<WPXProject>, label?: string) => void }) { return <div className="rounded-2xl border border-white/10 bg-[#21262D] p-4"><h2 className="font-bold text-white">Asset Manager UI</h2><input type="file" onChange={uploadAsset} className="mt-3" /><div className="mt-4 grid gap-3 md:grid-cols-3">{project.assets.map((asset) => <div key={asset.id} className="rounded-xl border border-white/10 bg-[#0D1117] p-3">{asset.previewUrl ? <img alt="asset preview" src={asset.previewUrl} className="h-24 w-full rounded object-cover" /> : <div className="grid h-24 place-items-center rounded bg-black/30 text-xs">No thumbnail</div>}<p className="mt-2 break-all text-xs text-white">{asset.localPath}</p><p className="text-xs text-[#8B949E]">{asset.mimeType} · {(asset.size / 1024).toFixed(1)} KB</p><button className="btn-danger mt-2" onClick={() => updateProject({ assets: project.assets.filter((item) => item.id !== asset.id) }, "Delete asset")}>Delete</button></div>)}</div></div>; }
